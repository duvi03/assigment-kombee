...
static void my_application_activate(
    ...
    MyApplication *
    self = MY_APPLICATION(application);
    GtkWindow *window =
            GTK_WINDOW(
...
#endif
    if (use_header_bar) {
        GtkHeaderBar *header_bar = GTK_HEADER_BAR(
                gtk_header_bar_new());
        gtk_widget_show(
                GTK_WIDGET(header_bar));
        gtk_header_bar_set_title(
                header_bar,
                "flutter_task_kombee_dhruvi_patel");
...
static void my_application_startup(
    ...
    G_APPLICATION_CLASS(
            my_application_parent_class)->startup(
            application);
}
...
static void my_application_shutdown(
    ...
    G_APPLICATION_CLASS(
            my_application_parent_class)->shutdown(
            application);
}
...
static void my_application_dispose(
        GObject *object) {
    MyApplication *
    self = MY_APPLICATION(object);
    g_clear_pointer(
            &self->dart_entrypoint_arguments,
            g_strfreev);
    G_OBJECT_CLASS(
            my_application_parent_class)->dispose(
            object);
}
...
static void my_application_class_init(
    ...
    G_APPLICATION_CLASS(
            klass)->local_command_line = my_application_local_command_line;
    G_APPLICATION_CLASS(
            klass)->startup = my_application_startup;
    G_APPLICATION_CLASS(
            klass)->shutdown = my_application_shutdown;
    G_OBJECT_CLASS(
